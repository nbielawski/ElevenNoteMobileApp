...
namespace ElevenNoteMobileApp.Services
{
    ...
    /// <summary>
    /// The real note service that will communicate with our webAPI
    /// </summary>
    public class RealNoteService : INoteService
    {
        ...
        public async Task<bool> Login(string username, string password)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var requestString = "grant_type=password" +
                    ...
                    var response = JsonConvert
                        .DeserializeObject<OauthBearerTokenResponse>(stringResponseFromServer);
        ...
        public async Task<List<NoteListItem>> GetAll()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var notes = JsonConvert
                    .DeserializeObject<List<NoteListItem>>(
        ...
        public async Task<bool> AddNew(NoteCreate note)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var result = await client.PostAsync(url,
                    new StringContent(json, Encoding.UTF8, "application/json")); // we have to specify we're sending JSON
